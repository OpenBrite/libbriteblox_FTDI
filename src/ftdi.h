...
#ifndef __libbriteblox_h__
#define __libbriteblox_h__

#include <stdint.h>
#include <sys/time.h>

/* 'interface' might be defined as a macro on Windows, so we need to
 * undefine it so as not to break the current libbriteblox API, because
 * struct briteblox_context has an 'interface' member
 * As this can be problematic if you include windows.h after briteblox.h
...
/** Parity mode for briteblox_set_line_property() */
enum briteblox_parity_type { NONE=0, ODD=1, EVEN=2, MARK=3, SPACE=4 };
/** Number of stop bits for briteblox_set_line_property() */
enum briteblox_stopbits_type { STOP_BIT_1=0, STOP_BIT_15=1, STOP_BIT_2=2 };
/** Number of bits for briteblox_set_line_property() */
enum briteblox_bits_type { BITS_7=7, BITS_8=8 };
/** Break type for briteblox_set_line_property2() */
enum briteblox_break_type { BREAK_OFF=0, BREAK_ON=1 };

/** MPSSE bitbang modes */
enum briteblox_mpsse_mode
{
...
enum briteblox_module_detach_mode
{
...
#define BRITEBLOX_DEVICE_OUT_REQTYPE (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE | LIBUSB_ENDPOINT_OUT)
#define BRITEBLOX_DEVICE_IN_REQTYPE (LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_DEVICE | LIBUSB_ENDPOINT_IN)
...
/* marker for unused usb urb structures
   (taken from libusb) */
#define BRITEBLOX_URB_USERCONTEXT_COOKIE ((void *)0x1)
...
struct briteblox_transfer_control
{
    int completed;
    unsigned char *buf;
    int size;
...
/**
    \brief Main context structure for all libbriteblox functions.

    Do not access directly if possible.
*/
struct briteblox_context
{
    /* USB specific */
    /** libusb's context */
    ...
    /* BRITEBLOX specific */
    /** BRITEBLOX chip type */
    enum briteblox_chip_type type;
    /** baudrate */
    int baudrate;
    /** bitbang mode state */
    unsigned char bitbang_enabled;
    /** pointer to read buffer for briteblox_read_data */
    ...
    /* BRITEBLOX FT2232C requirecments */
...
/**
    \brief list of usb devices created by briteblox_usb_find_all()
*/
struct briteblox_device_list
{
    /** pointer to next entry */
    struct briteblox_device_list *next;
...
enum briteblox_cbus_func  /* FIXME: Recheck value, especially the last */
{
...
enum briteblox_cbush_func  /* FIXME: Recheck value, especially the last */
{
...
typedef struct
{
    ...
    double totalRate;
    double currentRate;
} BRITEBLOXProgressInfo;

typedef int (BRITEBLOXStreamCallback)(uint8_t *buffer, int length,
                                 BRITEBLOXProgressInfo *progress, void *userdata);

/**
 * Provide libbriteblox version information
...
extern "C"
{
    ...
    void briteblox_set_usbdev (struct briteblox_context *briteblox, struct libusb_device_handle *usbdev);
    ...
    int briteblox_usb_open_dev(struct briteblox_context *briteblox, struct libusb_device *dev);
    ...
    int briteblox_readstream(struct briteblox_context *briteblox, BRITEBLOXStreamCallback *callback,
    ...
    int briteblox_set_event_char(struct briteblox_context *briteblox, unsigned char eventch, unsigned char enable);
    int briteblox_set_error_char(struct briteblox_context *briteblox, unsigned char errorch, unsigned char enable);

    /* init eeprom for the given BRITEBLOX type */
    int briteblox_eeprom_initdefaults(struct briteblox_context *briteblox,
    ...
    int briteblox_write_eeprom_location(struct briteblox_context *briteblox, int eeprom_addr, unsigned short eeprom_val);
