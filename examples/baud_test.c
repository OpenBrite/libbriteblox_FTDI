 ...
 *  -b <baudrate> (divides by 16 if bitbang as taken from the briteblox datasheets)
...
int main(int argc, char **argv)
{
    ...
    if (briteblox_usb_open_string(briteblox, devicedesc) < 0)
    {
    ...
    briteblox_set_baudrate(briteblox,set_baud);
    printf("real baudrate used: %d\n",(test_mode==BITMODE_RESET) ? briteblox->baudrate : briteblox->baudrate*16);

    if (briteblox_set_bitmode(briteblox, 0xFF,test_mode) < 0)
    {
    ...
    if (briteblox_write_data_set_chunksize(briteblox, txchunksize) < 0 ||
            briteblox_read_data_set_chunksize(briteblox, txchunksize) < 0)
    {
    ...
    if (test_mode==BITMODE_SYNCBB)
    {
        ...
        while (briteblox_read_data(briteblox, rxbuf, txchunksize)>0);
    }
    ...
    while (i < datasize)
    {
        ...
        if ((sendsize=briteblox_write_data(briteblox, txbuf, sendsize)) < 0)
        {
        ...
        if (test_mode==BITMODE_SYNCBB)
        {
            ...
            briteblox_read_data(briteblox, rxbuf, sendsize);
        }
    }
