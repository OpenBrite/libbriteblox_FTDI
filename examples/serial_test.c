...
int main(int argc, char **argv)
{
    ...
    {
        // Select interface
        briteblox_set_interface(briteblox, interface);
        
        // Open device
        f = briteblox_usb_open(briteblox, vid, pid);
    }
    if (f < 0)
    {
    ...
    // Set baudrate
    f = briteblox_set_baudrate(briteblox, baudrate);
    if (f < 0)
    {
     ...
     *
     */
    f = briteblox_set_line_property(briteblox, 8, STOP_BIT_1, NONE);
    if (f < 0)
    {
    ...
    {
        if (do_write)
            f = briteblox_write_data(briteblox, buf, 
                                (baudrate/512 >sizeof(buf))?sizeof(buf):
                                (baudrate/512)?baudrate/512:1);
        else
            f = briteblox_read_data(briteblox, buf, sizeof(buf));
        if (f<0)
            usleep(1 * 1000000);
