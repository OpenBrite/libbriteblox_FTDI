...
static int
readCallback(uint8_t *buffer, int length, BRITEBLOXProgressInfo *progress, void *userdata)
{
   if (length)
   {
       if (check)
...
int main(int argc, char **argv)
{
   ...
   if (briteblox_set_interface(briteblox, INTERFACE_A) < 0)
   {
       fprintf(stderr, "briteblox_set_interface failed\n");
   ...
   if(briteblox_set_latency_timer(briteblox, 2))
   {
...
/*   if(briteblox_usb_purge_rx_buffer(briteblox) < 0)
   {
   ...
   err = briteblox_readstream(briteblox, readCallback, NULL, 8, 256);
