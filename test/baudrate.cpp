...
extern "C" int convert_baudrate_UT_export(int baudrate, struct briteblox_context *briteblox,
                                 unsigned short *value, unsigned short *index);

/// Basic initialization of libbriteblox for every test
class BaseBRITEBLOXFixture
{
protected:
    briteblox_context *briteblox;

public:
    BaseBRITEBLOXFixture()
        : briteblox(NULL)
    {
        briteblox = briteblox_new();
    }

    virtual ~BaseBRITEBLOXFixture()
    {
        delete briteblox;
        briteblox = NULL;
    }
...
BOOST_FIXTURE_TEST_SUITE(Baudrate, BaseBRITEBLOXFixture)
 ...
 **/
static void test_baudrates(briteblox_context *briteblox, const map<int, calc_result> &baudrates)
{
    ...
    BOOST_FOREACH(const baudrate_type &baudrate, baudrates)
    {
        ...
        int calc_baudrate = convert_baudrate_UT_export(baudrate.first, briteblox, &calc_value, &calc_index);
...
BOOST_AUTO_TEST_CASE(TypeBMFixedBaudrates)
{
    ...
    BOOST_FOREACH(const enum briteblox_chip_type &test_chip_type, test_types)
    {
        briteblox->type = test_chip_type;
        test_baudrates(briteblox, baudrates);
    }
}
